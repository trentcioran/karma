...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
    ...
    showText : 'El formulario contiene errores (da clic para ver los detalles...)',
    
    hideText : 'Vuelve a dar clic para ocultar esta lista',
        ...
        sb.on('render', function(){
            ...
            if(this.form){
                ...
                this.form.on('beforeaction', function(f, action){
                    if(action.type == 'action.save' || action.type == 'action.update' ||
						action.type == 'action.base'){
...
Ext.ux.calendar.CONST = {
    ...
    BLANK_IMAGE_URL:'lib/ext/resources/images/default/s.gif',
    
    MAIN_PATH:'',
    
    CALENDAR_LANGUAGE_PATH:'lib/ext/locale/',
    
    EXT_LANGUAGE_PATH:'lib/ext/locale/',
...
Ext.ux.calendar.CalendarContainer = function(config){
    ...
    Ext.ux.calendar.CalendarContainer.superclass.constructor.call(this, {
        ...
        tbar: [this.backBtn, this.nextBtn, this.todayBtn, "-", lan["searchCriteria.text"], this.searchField, this.searchBtn, "->", this.refreshBtn, "-", this.dayBtn, "-", this.weekBtn, "-", this.monthBtn, "-", this.moreBtn]
    });
    ...
    showPressed: function(btn){
        ...
        if (btn == this.dayBtn) {
            ...
            if (btn == this.weekBtn) {
                ...
                if (btn == this.monthBtn || btn == this.monthOnlyBtn) {
                    this.currentIdx = 2;
...
Ext.ux.calendar.DayView = Ext.extend(Ext.ux.calendar.BasicView, {
    ...
    generateCSS: function(){
        var cssText = "." + this.id + "-x-dayview-viewer-row-height{}." + this.id + "-x-dayview-pool{overflow:hidden;}." + this.id + "-x-dayview-pool-height{}." + this.id + "-x-dayview-pool-width{}." + this.id + "-x-dayview-pool-viewer-width{}." + this.id + "-x-dayview-lefter-width{width:" + this.leftWidth + "px;}";
        var cid = Ext.id();
    ...
    generateHTML: function(data){
        ...
        for (var i = this.startRow; i < this.endRow; i++) {
            ...
            for (var j = 0; j < 1; j++) {
                ...
                if (0 != rest && i != this.activeStartRow) {
                    hour = '<div class="x-dayview-viewer-row-height" style="text-align:right;"><i><b class="x-dayview-lefter-inner x-dayview-lefter-fine-inner">' + hour + "</b></i></div>"
                }
                else {
                    hour = '<div class="x-dayview-viewer-row-height"><b class="x-dayview-lefter-inner">' + hour + "</b></div>"
                }
                lefter += '<td id="' + this.id + "-x-dayview-lefter-" + i + "-" + j + '" class="' + this.id + "-x-dayview-lefter-td x-dayview-lefter-row-height x-dayview-lefter-cell " + ((0 != (i + 1) % hnum) ? "x-dayview-lefter-odd-row" : "x-dayview-lefter-even-row") + '">' + hour + "</td>"
            }
            lefter += "</tr>"
        }
        var viewer = "";
        for (var i = this.startRow; i < this.endRow; i++) {
            ...
            for (var j = 0; j < this.dayNum; j++) {
                viewer += '<td class="' + this.id + "-x-dayview-viewer-td " + this.id + "-x-dayview-viewer-col-" + j + " " + ((0 != (i + 1) % hnum) ? "x-dayview-odd-row" : "x-dayview-even-row") + '"><div id="' + this.id + "-x-dayview-viewer-" + i + "-" + j + '" class="x-dayview-viewer-row-height x-dayview-viewer-cell"></div></td>'
            }
            viewer += "</tr>"
        }
        var port = '<div id="' + this.id + '-x-dayview-port" class="x-dayview-port x-dayview-inactive" unselectable="on" onselectstart="return false;"><div id="' + this.id + '-x-dayview-body" class="x-dayview-body x-dayview-inactive" unselectable="on" onselectstart="return false;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td class="x-dayview-lefter ' + this.id + '-x-dayview-lefter-width"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' + lefter + '</tbody></table></td><td class="x-dayview-viewer"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' + viewer + "</tbody></table></td></tr></tbody></table></div></div>";
        var days = "";
        this.dayTpl = new Ext.XTemplate('<tpl for="."><td class="x-dayview-header-days"><div class="' + this.id + '-x-dayview-pool-height"><b><u id="' + this.id + '-x-dayview-day-link-0-{idx}" class="x-dayview-header-day-link">{day}</u></b></div></td></tpl>');
        var d = [];
        for (var j = 0; j < this.dayNum; j++) {
            d[d.length] = {
                idx: j,
        ...
        var header = '<div id="' + this.id + '-x-dayview-header" class="x-dayview-header" unselectable="on" onselectstart="return false;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td class="x-dayview-header-lefter ' + this.id + '-x-dayview-lefter-width"><div class="x-dayview-wn">' + week + '</div></td><td class="x-dayview-header-viewer"><table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr>' + days + '</tr></tbody></table></td><td width="' + this.scrollOffset + 'px;"></td></tr></tbody></table></div>';
        var bg = "";
        for (var i = 0; i < this.dayNum; i++) {
            bg += '<td id="' + this.id + "-x-dayview-bg-0-" + i + '" class="x-dayview-bg-cell">&nbsp;</td>'
        }
        bg = '<table class="x-dayview-bg" width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr>' + bg + "</tr></tbody></table>";
        var pool = '<div id="' + this.id + '-x-dayview-pool" class="x-dayview-pool" unselectable="on" onselectstart="return false;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td class="x-dayview-header-lefter ' + this.id + '-x-dayview-lefter-width" style="vertical-align:top;"><div id="' + this.id + '-x-dayview-pool-switch" class="x-dayview-pool-collapse"></div></td><td><div id="' + this.id + '-x-dayview-pool-viewer" class="' + this.id + '-x-dayview-pool-width x-dayview-pool-viewer">' + bg + '<table id="' + this.id + '-x-dayview-pool-ct" width="100%" class="x-dayview-ct" cellspacing="0" cellpadding="0" border="0"><tbody><tr></tr></tbody></table><div class="x-dayview-pool-rest"></div></div></td><td width="' + this.scrollOffset + 'px;"></tr></tbody></table></div>';
    ...
    initComponent: function(){
        ...
        this.addEvents("checklayout", "sizechanged", "afterresize", "beforeremoteload", "remoteload", "canceldetail", "viewDay", "viewWeek");
        ...
        if (nol == col) {
            ...
            if ("string" == Ext.type(oevent.repeatType)) {
                eh.ds.updateEvent(event, function(){
                    var layout = eh.calendarLayout.getLayout(cview.daySet[col], cview);
                    var rs = layout.updateLayout(oevent, "delete");
                    eh.renderEvent(cview, rs.elist, col);
                    layout = eh.calendarLayout.getLayout(day, cview);
        ...
        if (0 == e.button) {
            ...
            if (tgEl.hasClass("x-dayview-viewer-cell")) {
                ...
                if (tgEl.hasClass("x-event-bottom-default")) {
                    ...
                    if (!tgEl.hasClass("x-event-pin") && !tgEl.hasClass("x-event-content-link")) {
                        delete (this.preEl);
                        var eEl = tgEl;
    ...
    handleResize: function(bw, bh, still){
        if (typeof bw == "number") {
            ...
            var r = pw % 7;
            this.leftWidth = 70 + r;
            Ext.util.CSS.updateRule("." + this.id + "-x-dayview-lefter-width", "width", this.leftWidth + "px");
...
Ext.ux.calendar.DetailEditor = function(config){
    ...
    var tpl = '<tpl for="."><div class="x-combo-list-item">' + this.ehandler.cTplStr + "</div></tpl>";
    ...
    this.generalForm = new Ext.form.FormPanel({
        ...
        autoHeight: true,
        labelWidth: 80,
        items: [this.timepanel, this.subjectField, this.contentField, this.calendarField, this.alertCB, this.lockCB]
    });
...
Ext.ux.calendar.EventHandler = function(config){
    ...
    this.detailTpl = new Ext.XTemplate('<div class="x-event-detail-ct"><div class="x-event-detail-title"><table width="100%" border="0"><tbody><tr><td class="x-event-detail-title-td">{title}</td><td class="x-event-detail-tool"><img class="x-event-detail-tool-close" src="' + Ext.BLANK_IMAGE_URL + '"></img></td></tr></tbody></table></div><div class="x-event-detail-viewer"></div><div class="x-event-detail-foot"><table width="100%" border="0"><tbody><tr><td class="x-event-detail-foot-tool"><img class="x-event-detail-foot-info" src="' + Ext.BLANK_IMAGE_URL + '"></img></td><td><b class="x-event-detail-foot-text"></b></td></tr></tbody></table></div></div>');
    this.eventTpl = new Ext.XTemplate('<div eid="{id}" name="x-event-{day}-{eday}-{id}" class="' + this.id + "-x-calendar-{calendarId}-event x-event-cover " + this.id + '-x-calendar-{calendarId} x-calendar-event" style="{cover-style}" unselectable="on" onselectstart="return false;"><div style="height:18px;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td style="{left-style}"><div class="x-calendar-{color}-event-ltcorner-clear  x-event-lt-default">&nbsp;</div></td><td><div class="x-calendar-{color}-event-top-clear x-event-inner x-event-title-default" style="{title-style}"><b qtip="{time}<br><b><u>{subject}</u></b><br>{content}">{title}</b></div></td><td style="{right-style}"><div class="x-calendar-{color}-event-rtcorner-clear  x-event-rt-default">&nbsp;</div></td></tr></tbody></table></div><div class="x-calendar-{color}-event-lr x-event-content-default" style="{content-style}"><img class="x-calendar-event-pin-off x-event-pin" src="' + Ext.BLANK_IMAGE_URL + '"></img><tpl if="this.isRepeat(repeatType)"><img class="x-repeat-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isException(repeatType)"><img class="x-exception-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isAlert(alertFlag)"><img class="x-alert-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isLocked(locked)"><img class="x-locked-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><u class="x-event-content-link" qtip="{time}<br><b><u>{subject}</u></b><br>{content}">{subject}</u><br>{content}</div><div><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td style="{left-style}"><div class="x-calendar-{color}-event-lbcorner">&nbsp;</div></td><td><div style="{bottom-style}" class="x-calendar-{color}-event-bottom x-event-bottom-default">&nbsp;</div></td><td style="{right-style}"><div class="x-calendar-{color}-event-rbcorner">&nbsp;</div></td></tr></tbody></table></div></div>', {
    ...
    this.legendTpl = new Ext.XTemplate('<div eid="{id}" name="x-event-{day}-{eday}-{id}" class="' + this.id + "-x-calendar-{calendarId}-legend x-legend-cover " + this.id + '-x-calendar-{calendarId} x-calendar-event" unselectable="on" onselectstart="return false;"><b class="x-legend-title-b x-legend-title-{color}" qtip="{time}<br><b><u>{subject}</u></b><br>{content}"><tpl if="this.isRepeat(repeatType)"><img class="x-repeat-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isException(repeatType)"><img class="x-exception-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isAlert(alertFlag)"><img class="x-alert-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isLocked(locked)"><img class="x-locked-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl>{title} {subject}</b></div>', {
        isRepeat: function(repeatType){
            return ("string" != Ext.type(repeatType))
        },
        isException: function(repeatType){
    ...
    this.wholeTpl = new Ext.XTemplate('<div eid="{id}" name="x-event-{day}-{eday}-{id}" class="' + this.id + "-x-calendar-{calendarId}-whole x-whole-cover " + this.id + '-x-calendar-{calendarId} x-calendar-event" unselectable="on" onselectstart="return false;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td class="x-whole-td"><div class="x-calendar-{color}-whole-left x-whole-left">&nbsp;</div><tpl if="this.isLeftJoin(lflag)"><div class="x-whole-left-join"></div></tpl></td><td class="x-whole-title-{color} x-whole-title">&nbsp;<b class="x-whole-title-b" qtip="{time}<br><b><u>{subject}</u></b><br>{content}"><tpl if="this.isRepeat(repeatType)"><img class="x-repeat-white-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isException(repeatType)"><img class="x-exception-white-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isAlert(alertFlag)"><img class="x-alert-white-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl><tpl if="this.isLocked(locked)"><img class="x-locked-white-event" src="' + Ext.BLANK_IMAGE_URL + '"></img> </tpl>{subject}</b></td><td class="x-whole-td"><div class="x-calendar-{color}-whole-right x-whole-right">&nbsp;</div><tpl if="this.isRightJoin(rflag)"><div class="x-whole-right-join"></div></tpl></td></tr></tbody></table></div>', {
    ...
    this.calendarDropTpl = new Ext.XTemplate('<div class="x-calendardrop-cover" hidefocus="true" unselectable="on" onselectstart="return false;"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td>' + this.cTplStr + '</td><td class="x-legend-tool-td"><div class="x-legend-tool"></div></td></tr></tbody></table><div>');
    this.calendarDropTpl.compile();
    this.calendarTpl = new Ext.XTemplate(this.cTplStr);
    this.calendarTpl.compile();
    this.initMenu();
    this.initContextMenu();
    this.addEvents("calendarloaded", "reloadCalendar", "createEvent", "updateEvent", "deleteEvent", "changeDay", "deleteCalendar", "clearCalendar", "changeEventCache", "changeCalendarColor");
...
Ext.extend(Ext.ux.calendar.EventHandler, Ext.util.Observable, {
    ...
    initMenu: function(){
        ...
        this.menu = new Ext.menu.Menu({
            cls: "x-calendar-menu",
            items: [this.showOnlyItem, this.viewItem, this.editItem, this.deleteItem, this.clearItem, "-", palette]
        });
    ...
    initContextMenu: function(){
        ...
        this.cmenu = new Ext.menu.Menu({
            items: [this.lockItem, this.unlockItem, this.editItem, this.deleteItem]
        });
    ...
    generateTitle: function(event){
        ...
        if (0 == startRow && this.rowCount == endRow) {
            ...
            var hm = Ext.ux.calendar.Mask.getIntervalFromRow(this.intervalSlot, startRow, this.hourFormat) + "-" + Ext.ux.calendar.Mask.getIntervalFromRow(this.intervalSlot, endRow, this.hourFormat);
            return hm
        }
    },
    ...
    getStartDateInWeek: function(date, cview){
        ...
        if (1 == this.startDay) {
            ...
            var w = -date.format("w");
            if ((5 == cview.dayNum && cview instanceof Ext.ux.calendar.DayView) || (5 == cview.colNum && cview instanceof Ext.ux.calendar.MonthView)) {
                w++
            }
            sDate = date.add(Date.DAY, w)
        }
...
Ext.ux.calendar.ExpirePopup = function(config){
    ...
    this.tpl = new Ext.XTemplate('<tpl for="."><div style="background-color:{color};padding:5px;border-bottom:1px solid white;"><div><table width="' + p + '%"><tbody><tr><td style="font-size:11px;"><b>From: {startDate}</b><br><b>To: </b><b>{endDate}</b></td><td style="text-align:right;font-size:11px;">' + lan["tpl.calendar"] + ": <b>{calendar}</b></td></tr></tbody></table></div>" + lan["tpl.subject"] + ": <b><u>{subject}</u></b><br>" + lan["tpl.content"] + ": {content}<br>" + lan["tpl.leftTime"] + ": <b>{left}</b></div></tpl>");
    this.tpl.compile();
...
Ext.extend(Ext.ux.calendar.CalendarLayout, Ext.util.Observable, {
    ...
    combine2List: function(alist, blist){
        ...
        for (i = 0, j = 0; i < alen && j < blen;) {
            var a = alist[i];
            var b = blist[j];
            var ast = a.day + "-" + a.startRow, aet = a.eday + "-" + a.endRow;
            var bst = b.day + "-" + b.startRow, bet = b.eday + "-" + b.endRow;
            ...
            for (var j = 0; j < colNum;) {
                if (!row[j]) {
                    ...
                    if ("number" == Ext.type(e)) {
                        ...
                        html = '<div class="x-event-more-ct"><u><b name="' + day + '" class="x-event-more">&nbsp;&nbsp;&nbsp;&nbsp;' + e + " " + lan.more + "...</b></u></div>"
                    }
                    else {
...
Ext.ux.calendar.MonthView = Ext.extend(Ext.ux.calendar.BasicView, {
    ...
    initMenu: function(){
        ...
        this.menu = new Ext.menu.Menu({
            items: [this.addItem, this.clearItem, "-", this.cutItem, this.copyItem, this.pasteItem]
        });
        this.menu = Ext.menu.MenuMgr.get(this.menu)
    },
    getCellIndex: function(cellId){
    ...
    generateHTML: function(data){
        this.generateCSS();
        this.viewerTpl = new Ext.XTemplate('<div id="' + this.id + '-x-monthview-viewer"><table width="100%" cellspacing="0" cellpadding="0" border="0" unselectable="on" onselectstart="return false;"><tbody><tr><td class="x-monthview-lefter"><div id="' + this.id + '-x-monthview-lefter"><tpl for="."><div class="x-monthview-lefter-inner ' + this.id + '-x-monthview-row"><u><b id="' + this.id + '-x-monthview-week-{idx}-0" class="x-monthview-lefter-inner-b">{week}</b></u></div></tpl></div></td><td class="x-monthview-port-td"><div id="' + this.id + '-x-monthview-port" class="x-monthview-port"><tpl for="."><div class="' + this.id + '-x-monthview-row x-monthview-row" unselectable="on"><table class="x-monthview-bg" width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><tpl for="arr"><td id="' + this.id + '-x-monthview-viewer-{parent.idx}-{idx}" class="' + this.id + '-x-monthview-viewer-col-{idx} x-monthview-viewer-cell">&nbsp;</td></tpl></tr></tbody></table><table id="' + this.id + '-x-monthview-ct-{idx}" class="x-monthview-ct" width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr><tpl for="arr"><td id="' + this.id + '-x-monthview-viewer-title-{parent.idx}-{idx}" class="' + this.id + '-x-monthview-viewer-col-{idx} x-monthview-viewer-title"><div><table width="100%" height="100%" border="0"><tbody><tr><td><u id="' + this.id + '-x-monthview-viewer-link-{parent.idx}-{idx}" class="x-monthview-viewer-link"><b class="x-monthview-viewer-link-b">{day}</b></u></td>' + (this.ehandler.readOnly ? "" : '<td class="x-monthview-viewer-tool"><img id="' + this.id + '-x-monthview-tool-add-{parent.idx}-{idx}" class="x-monthview-tool-add" src="' + Ext.BLANK_IMAGE_URL + '"></img><img id="' + this.id + '-x-monthview-tool-drop-{parent.idx}-{idx}" class="x-monthview-tool-drop" src="' + Ext.BLANK_IMAGE_URL + '"></img></td>') + "</tr></tbody></table></div></td></tpl></tr></tbody></table></div></tpl></div></div></td></tr></tbody></table>");
        ...
        var days = "";
        for (var j = 0; j < this.dayNum; j++) {
            days += '<td class="' + this.id + "-x-monthview-viewer-col-" + j + ' x-dayview-header-days"><div><b style="line-height:15px;">' + Ext.ux.calendar.Mask.MonthView.dayPre + this.daySet[j].format("l") + "</b></div></td>"
        }
        var header = '<div id="' + this.id + '-x-monthview-header" class="x-monthview-header" unselectable="on"><table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr>' + days + "</tr></tbody></table></div>";
        var html = header + viewer;
        return html
    },
    generateCSS: function(){
        var cssText = "." + this.id + "-x-monthview-viewer-row-height{}." + this.id + "-x-monthview-row{}." + this.id + "-x-monthview-viewer-list{}";
        for (var i = 0; i < this.dayNum; i++) {
    ...
    showDetails: function(day){
        ...
        this.detailTitle.dom.innerHTML = '<u id="' + Ext.id() + "-x-monthview-viewer-link-" + x + "-" + y + '" class="x-monthview-viewer-link"><b class="x-monthview-viewer-link-b">' + day + "</b></u>";
        this.detailFoot.dom.innerHTML = events.length + " " + lan.events;
        eh.bindEvent2Detail(this, events, this.detailViewer);
        this.alignDetailCt()
    },
    ...
    onRowClickFn: function(grid, rowIndex, e){
        ...
        if (tgEl.hasClass("x-calendar-resultview-lock")) {
            if (tgEl.hasClass("icon_feyaCalendar_event_lock")) {
                ...
                if (tgEl.hasClass("icon_feyaCalendar_event_unlock")) {
...
Ext.ux.calendar.SettingPopup = function(config){
    ...
    this.generalForm = new Ext.form.FormPanel({
        ...
        bodyStyle: "background:transparent;",
        labelWidth: 180,
        items: [this.hourFormatField, this.fromtoFormatField, this.startDayField, this.readOnlyField, this.languageField]
    });
    this.dwViewForm = new Ext.form.FormPanel({
        ...
        bodyStyle: "background:transparent;",
        labelWidth: 180,
        items: [this.dayFormatField, this.weekFormatField, this.createByDblClickField, this.singleDayField, this.activeStartTimeField, this.activeEndTimeField, this.hideInactiveTimeField, this.intervalField]
    });
    ...
    onApplyFn: function(){
        ...
        if (flag) {
            ...
            eh.ds.updateSetting(params, function(backObj){
                if (eh.language != language || eh.createByDblclick != createByDblclick || eh.singleDay != singleDay || eh.activeStartTime != activeStartTime || eh.activeEndTime != activeEndTime || eh.hideInactiveRow != hideInactiveRow || eh.intervalSlot != intervalSlot || eh.startDay != startDay || eh.readOnly != readOnly) {
                    window.location = window.location.href;
                    return
                }
                var cs = eh.calendarSetting;
...
Ext.extend(Ext.ux.calendar.WestPanel, Ext.Panel, {
    onAfterLayoutFn: function(){
        if (this.poweredBy.charAt(13) == "y" && this.poweredBy.charAt(17) == "f") {
        }
        else {
